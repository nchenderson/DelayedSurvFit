plot.surv.delay <- function(obj, type="surv", ...) {
  ## plotting function
  if(type=="surv") {
    plot(obj$times, obj$surv0, type="n", las=1, xlab="Time", ylab="Survival Prob.",
         xlim=c(0, max(obj$times)), ylim=c(0,1), ...)
    lines(c(0,obj$times), c(1, obj$surv0), type="s", lwd=2)
    lines(c(0,obj$times), c(1, obj$surv1), type="s", col="red", lwd=2)
