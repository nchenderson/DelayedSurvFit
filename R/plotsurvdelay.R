plot.surv.delay <- function(x, ..., type="surv") {
  ...
  if(type=="surv") {
     ...
     legend("bottomleft", legend=c("Control Arm", "Active Treatment Arm"), col=c("black", "red"), lwd=3,
           bty='n')
     ...
     abline(v=x$theta, lwd=2)
     legend("topleft", legend=c("Control Arm", "Active Treatment Arm"), col=c("black", "red"), lwd=3,
            bty='n')
